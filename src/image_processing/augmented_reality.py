...
from image_processing.utils import load_gif, overlay_images
...
def render_frame(raw_img : np.ndarray, frame_num : int, detections : List[int], max_frames_length: int) -> np.ndarray:
    """
...
def run_ar_pipeline_for_single_img(img_path : str) -> None:
    ...
    for frame_id in range(MAX_FRAMES):
        ...
        if frame_id < len(_internal_loading_gif_frame):
            ...
            animated_img = overlay_images(img_raw.copy(), overlay, position=(50, 50)) 
        ...
        for i in range(len(predictions['bbox'])):
            ...
            animated_img = render_frame(animated_img, frame_id, bbox, MAX_FRAMES)
            # Text Animation
